...
const LoteForm: React.FC<LoteFormProps> = ({ lote, onClose, onSave }) => {
  ...
  useEffect(() => {
    ...
    if (lote) {
      // Modo Edición - deshabilitar modo masivo
      setIsBulkMode(false);
      setFormData({
        block: lote.block,
        ...
        contract_file: null,
        contract_date: lote.contract_date
          ? lote.contract_date.slice(0, 16)
  ...
  const handleBulkDataChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setBulkData((prev) => ({
      ...prev,
      [name]: name.includes("Number") ? parseInt(value) || 1 : value,
    }));
  };
  ...
  const handleDownloadContract = () => {
    if (formData.existing_contract_file) {
      ...
      link.target = "_blank";
      link.download =
        formData.existing_contract_file.split("/").pop() || "contrato.pdf";
    ...
    try {
      if (isBulkMode && !lote) {
        ...
        for (let i = startLotNumber; i <= endLotNumber; i++) {
          const lotNumber = lotNumberPrefix
            ? `${lotNumberPrefix}${i}`
            : i.toString();
          lotesToCreate.push({
        ...
        const shouldDeleteContract =
          !formData.existing_contract_file && !hasFile;
        ...
        if (hasFile) {
          ...
          formDataToSend.append(
            "financing_months",
            formData.financing_months.toString()
          );
          formDataToSend.append("payment_day", formData.payment_day.toString());
          formDataToSend.append(
            "owner_id",
            formData.owner_id ? formData.owner_id.toString() : ""
          );
          ...
          // usar siempre el mismo endpoint de create/update con multipart
          if (lote) {
            ...
            {isBulkMode && !lote && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-4">
                ...
                <div className="text-sm text-blue-700 bg-blue-100 p-3 rounded-lg">
                  ...
                  {Math.max(
                    0,
                    bulkData.endLotNumber - bulkData.startLotNumber + 1
                  )}{" "}
                  lotes
                  {bulkData.startLotNumber <= bulkData.endLotNumber && (
                    <>
                      <br />
                      <strong>Ejemplos:</strong> {bulkData.lotNumberPrefix}
                      {bulkData.startLotNumber}, {bulkData.lotNumberPrefix}
                      {bulkData.startLotNumber + 1}, ...,{" "}
                      {bulkData.lotNumberPrefix}
                      {bulkData.endLotNumber}
                    </>
                  )}
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  value={Number(formData.financing_months)}
                  onChange={handleChange}
                  className="w-full p-3  border border-gray-300 rounded-lg "
                />
              </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-sm text-green-800">
                            Contrato existente:{" "}
                            {formData.existing_contract_file.split("/").pop()}
                          </span>
                        </div>
                        <div className="flex items-center space-x-2">
                          <button
            ...
            >
              {isSubmitting ? (
                <>
                  ...
                  <span>
                    {isBulkMode ? "Creando Lotes..." : "Guardando..."}
                  </span>
                </>
              ) : (
                <>
